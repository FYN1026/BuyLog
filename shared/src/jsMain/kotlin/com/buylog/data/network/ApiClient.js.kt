...
actual class HaodankuApiClient actual constructor() {
    ...
    actual suspend fun fetchProductInfo(content: String): Product? {
        return try {
            ...
            val params = mapOf("method" to apiMethod, "app_id" to appId, "date" to date, "content" to content)
            ...
            val response = fetch(apiUrl, json("method" to "POST", "headers" to json("Content-Type" to "application/json"), "body" to body)).await<Response>()
    ...
    private fun buildJsonString(map: Map<String, String>): String {
        val entries = map.entries.joinToString(",") { (k, v) -> "\"$k\":\"${v.replace("\"", "\\\"")}\"" }
        return "{$entries}"
    }
}
